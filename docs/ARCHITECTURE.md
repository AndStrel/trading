...
Годовые минутные архивы T-Invest импортируются только явной CLI-командой в отдельный `market-data.sqlite`, не в журнал сценариев. Для каждого импорта фиксируются год, SHA-256 архива, число исходных/отброшенных строк и время получения; свеча адресуется сочетанием UID инструмента и UTC-времени. Это позволяет повторно загрузить обновлённый архив без дублей и позднее воспроизводимо проверить правила. Архив не содержит текущий день, поэтому он не используется для принятия текущего решения и позднее будет дополняться live-данными. Подробнее: [исторические данные](HISTORICAL_DATA.md).
...
Локальный процесс `scan:intraday` не открывает брокерские или paper-позиции. В штатном режиме он раз в сутки получает `InstrumentsService/Shares`, разрешает заранее проверенное ядро ликвидных тикеров только на основной доске TQBR в RUB и автоматически берёт UID, лот и шаг цены. Каждые пять минут он обрабатывает эту вселенную с ограниченной параллельностью, отсекает недостаточный rolling 5-минутный оборот, ранжирует ситуации и только затем читает стакан/статус для сильных сигналов. Он сохраняет `candidate` не чаще заданного cooldown и не более заданного числа за проход. Условия первой эвристики прозрачны и намеренно просты: 5-минутный `up`-тренд, относительный объём от 1.0, стоп от ATR, цель 2.5R. Это механизм сбора paper-выборки, а не доказанная стратегия.
