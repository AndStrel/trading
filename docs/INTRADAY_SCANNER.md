...
Сканер работает на вашем компьютере или VPS и раз в пять минут проверяет только инструменты из `T_INVEST_INTRADAY_WATCHLIST`. Он использует 5-минутные свечи за последние шесть часов, стакан, последнюю цену и статус торгов.
