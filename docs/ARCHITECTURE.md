...
Telegram-бот, сканер и журнал не получают API команд на открытие paper- или реальных сделок. Позиция чтения обновлений хранится в SQLite, поэтому после перезапуска уже обработанные команды не повторяются.
